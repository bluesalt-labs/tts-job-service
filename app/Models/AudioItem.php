...
class AudioItem extends Model
{
     ...
     * @return array
     */
    public static function createJob($text, $voices, $outputFormat = 'mp3', $name = null, User $user = null) {
        $output = [
            'items'     => [],
            'messages'  => [],
        ];
        ...
        if(sizeof($voiceIDs) > 0) {
            ...
            foreach($voiceIDs as $voice) {
                $ttsItem = new AudioItem([
                    'name'          => $name,
                    'user_id'       => ($user ? $user->getKey() : null),
                    'status'        => AudioItem::STATUS_DEFAULT,
                    'voice_id'      => $voice,
    ...
    /**
     * Determine if the job is pending (currently being processed).
     */
        ...
        foreach($textParts as $key => $requestString) {
            try {
                ...
                $this->setStatusAndMessage(
                    ...
                    ($stored ? 'info' : 'error'),
                    ($stored ? null : static::STATUS_FAILED)
                );
